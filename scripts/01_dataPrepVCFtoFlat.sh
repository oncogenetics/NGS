...
#echo -e "CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tAF\tCLNSIG" > ${fileOut}.CLNSIG
#bcftools query -f '%CHROM\t%POS\t%ID\t%REF\t%ALT\t%QUAL\t%FILTER\t%AF\t%INFO/CLNSIG\n' ${fileVCF} >> ${fileOut}.CLNSIG
...
fileOut=${folderOut}/`basename ${fileVCF}`
echo -e "CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tAF\tCLNSIG" > ${fileOut}.CLNSIG
bcftools +fill-tags ${fileVCF} -- -t AF | bcftools query -f '%CHROM\t%POS\t%ID\t%REF\t%ALT\t%QUAL\t%FILTER\t%AF\t%INFO/CLNSIG\n' >> ${fileOut}.CLNSIG
done
