...
GT <- rbindlist(
  lapply(list.files("data/ShinyAppInput/", pattern = "*.GT", full.names = TRUE), function(i){
    ...
    d[ , (s) := lapply(.SD, convertGT), .SDcols = s ]
    
    # keep only cases that are in progeny (in pheno objcet)
    d <- d[, colnames(d) %in% pheno$Study.ID, with = FALSE ]
...
# some vars and samples duplicated over VCFs, so deduplicate if calls are the same
GT <- unique(GT)

# to-do:get max when discordant, temp solution, will need to change...
# GT <- GT[ , (value = max(value)), by = .(varname, sampleID) ]
# when genotypes are discordant, then exclude.
ix <- which(duplicated(GT[, 1:2]))
GT <- GT[ !(sampleID %in% unique(GT[ ix, sampleID ]) &
              varname %in% unique(GT[ ix, varname ])), ]
...
GTalleleCounts[, AF:=(BB * 2 + AB)/((AA + AB + BB) * 2) ]

#GTalleleCounts[ which(GTalleleCounts$AF > 0.5), ]
cols = colnames(GT)[ -1 ]
GT[which(GTalleleCounts$AF > 0.5), (cols):= lapply(.SD, function(i){ 2L - i }), .SDcols = cols ]
...
# drop monomorphic vars
dim(GT)
GT <- GT[ !(GTalleleCounts$AF == 0 | GTalleleCounts$AF == 1), ]
dim(GT)
...
# output ------------------------------------------------------------------
save(ANNOT, GT, SAMPLE, geneListVCF, geneListPanel,
